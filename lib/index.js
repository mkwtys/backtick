...
async function backtick(src, dest, data, opt) {
  const options = opt || {};
  const base = options.base || globParent(src);
  ...
  return Promise.all(
    templates.map((template) => {
      return Promise.resolve().then(() => {
        ...
        const normalizedTemplate = template.split(path.sep).join("/");
        const normalizedBase = base.split(path.sep).join("/");
        const filePath = path.resolve(dest, normalizedTemplate.replace(normalizedBase, ""));
